...
export function parseGANGNAMText(text) {
  ...
  // 시·도 및 시·군·구 두 단계만 뽑아냄 (특별자치도/특별시/광역시/도, 시/군/구 제거)
  ...
  // 발표일
  const announcementDate = clean(
    text.match(/리뷰어 발표\s*([0-9.]+|[0-9]+월\s*[0-9]+일|[0-9]{2}\.[0-9]{2})/)?.[1] ||
    ''
  );

  // 리뷰 등록기간 (체험기간)
  const expMatch = text.match(/리뷰 등록기간\s*([0-9.]+)\s*[~\-]\s*([0-9.]+)/);
  const experiencePeriod = expMatch
    ? `${clean(expMatch[1])} ~ ${clean(expMatch[2])}`
    : '';

  // 네이버 플레이스 URL
  const naverPlaceUrl = clean(
    text.match(/https?:\/\/map\.naver\.com\/v5\/search\/[^"]+/)?.[0] ||
    ''
  );
